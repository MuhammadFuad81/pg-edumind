(() => {
  ...
  const APP_VERSION = 'PG-RKJM-1.1.0';
  ...
  function enterApp() {
    ...
    // v1.0.3: bila draft ada, pulihkan otomatis ke langkah terakhir.
    // Ini mencegah pengguna tanpa sengaja memulai draft baru setelah refresh/login ulang.
    if (draft) {
      state = migrateState(draft);
      openWorkspace(state.app.currentStep || 1);
      if ($('autosaveStatus')) $('autosaveStatus').textContent = `Draft dipulihkan ✓${state.app.updatedAt ? ' · ' + formatDraftTime(state.app.updatedAt) : ''}`;
      setTimeout(() => toast('Draft terakhir dipulihkan otomatis ✓'), 50);
      return;
    }

    $('welcomeView').classList.remove('app-hidden');
    $('resumeDraftWelcome').classList.add('hidden');
  }
  ...
  function bindStaticEvents() {
    ...
    $('startWizard').addEventListener('click', () => {
      if (loadDraft(false)) confirmDialog('Mulai draft baru?','Draft tersimpan ditemukan. Memulai baru akan menghapus draft utama dan cadangan pada perangkat ini.',()=>{clearDraft();state=DEFAULT_STATE();hydrateAll();openWorkspace(1);},'Ya, Mulai Baru');
    ...
    $('appMenu').addEventListener('click',(e)=>{
      ...
      if(action==='export-draft')exportDraft(); if(action==='import-draft')$('importDraftInput').click();
    });
