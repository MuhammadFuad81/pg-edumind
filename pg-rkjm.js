(() => {
  ...
  const BACKUP_KEY = 'edumind_pg_rkjm_v1_backup';
  const ARCHIVE_KEY = 'edumind_pg_rkjm_v1_archive';
  ...
  const DEFAULT_STATE = () => ({
    ...
    evidence: [],
    swot: { mode: 'NONE', strengths: '', weaknesses: '', opportunities: '', threats: '', strategies: 'NO' },
    priorities: [],
    objectives: [],
    programs: [],
    domains: {
  ...
  function renderDraftRecovery(draft) {
    ...
    const resume = $('resumeDraftWelcome');
    const start = $('startWizard');
    const notice = $('draftRecoveryNotice');
    const meta = $('draftRecoveryMeta');
    resume.classList.toggle('hidden', !hasDraft);
    notice.classList.toggle('hidden', !hasDraft);
    start.textContent = hasDraft ? 'Mulai Draft Baru' : 'Mulai';
    start.classList.toggle('btn-primary', !hasDraft);
    start.classList.toggle('btn-danger', hasDraft);
    resume.classList.toggle('btn-primary', hasDraft);
    resume.classList.toggle('btn-secondary', !hasDraft);
    if (hasDraft) {
      const p = draft.profile || {};
      const bits = [safeText(p.unitName) || 'Nama satuan belum diisi', safeText(p.educationLevel), safeText(p.ministry)==='KEMENAG'?'Kemenag':safeText(p.ministry)==='KEMENDIKDASMEN'?'Kemendikdasmen':''].filter(Boolean);
      const step = Math.max(1, Math.min(8, Number(draft.app && draft.app.currentStep) || 1));
      const when = draft.app && draft.app.updatedAt ? formatDraftTime(draft.app.updatedAt) : '';
      meta.textContent = `${bits.join(' · ')} · terakhir di Langkah ${step}${when ? ' · tersimpan ' + when : ''}.`;
  ...
  function bindStaticEvents() {
    ...
    $('loginForm').addEventListener('submit', (e) => {
      e.preventDefault();
      const msg = $('loginMessage'); msg.className = 'login-message';
      if (!AUTH_CONFIG.username || !AUTH_CONFIG.password) { msg.textContent='Credential PG RKJM belum dikonfigurasi.'; msg.classList.add('show','info'); return; }
      if ($('loginUsername').value === AUTH_CONFIG.username && $('loginPassword').value === AUTH_CONFIG.password) {
        ...
        msg.className='login-message'; enterApp();
      } else { msg.textContent='Username atau password tidak sesuai. Periksa kembali data login Anda.'; msg.classList.add('show','error'); }
    });
    ...
    $('resumeDraftWelcome').addEventListener('click',()=>{
      ...
      openWorkspace(state.app.currentStep||1);
      if ($('autosaveStatus')) $('autosaveStatus').textContent=`Draft dilanjutkan ✓${state.app.updatedAt ? ' · ' + formatDraftTime(state.app.updatedAt) : ''}`;
    ...
    $('appMenu').addEventListener('click',(e)=>{
      const action=e.target.dataset.action;if(!action)return;$('appMenu').classList.add('hidden');
      if(action==='restart')restartFlow(); if(action==='logout')logout(); if(action==='about')showAbout(); if(action==='open-regulation')showRegulation();
      if(action==='export-draft')exportDraft(); if(action==='import-draft')$('importDraftInput').click(); if(action==='recover-previous')recoverPreviousDraft();
    });
    ...
    $('previewPrompt').addEventListener('click',()=>previewPrompt(false)); $('generatePrompt').addEventListener('click',generatePrompt); $('copyPrompt').addEventListener('click',copyFinalPrompt); $('previewPromptResult').addEventListener('click',()=>showPromptModal(compiledPrompt)); $('editData').addEventListener('click',()=>openWorkspace(8)); $('restartResult').addEventListener('click',restartFlow);
    $('closeModal').addEventListener('click',closeModal); $('modalBackdrop').addEventListener('click',(e)=>{if(e.target===$('modalBackdrop'))closeModal();}); document.addEventListener('keydown',(e)=>{if(e.key==='Escape'&&!$('modalBackdrop').classList.contains('hidden'))closeModal();});
  }
  ...
  function handleGlobalChange(e) {
    ...
    if (e.target.name === 'swotMode') state.swot.mode = e.target.value;
    if (e.target.id === 'swotStrategies') state.swot.strategies = e.target.value;
    if (e.target.id === 'includeApproval') { state.output.includeApproval=e.target.value; $('approvalFields').classList.toggle('hidden',e.target.value!=='YES'); }
    updateChoiceCards(); collectStaticFields(); renderSwotUi(); renderAllDerived(); scheduleSave();
  }
  ...
  function collectStaticFields() {
    ...
    const map={
      ...
      swotStrengths:['swot','strengths'],swotWeaknesses:['swot','weaknesses'],swotOpportunities:['swot','opportunities'],swotThreats:['swot','threats'],swotStrategies:['swot','strategies'],
    ...
    Object.entries(map).forEach(([id,path])=>{const el=$(id);if(el)state[path[0]][path[1]]=el.value;}); calculatePeriod(false);
  }

  function hydrateStaticFields() {
    const map={educationLevel:state.profile.educationLevel,unitName:state.profile.unitName,npsn:state.profile.npsn,nsm:state.profile.nsm,unitStatus:state.profile.unitStatus,location:state.profile.location,rkjmStart:state.profile.rkjmStart,rkjmEnd:state.profile.rkjmEnd,preparationYear:state.profile.preparationYear,headName:state.profile.headName,vision:state.strategy.vision,missions:state.strategy.missions,goals:state.strategy.goals,unitContext:state.strategy.unitContext,stakeholderNeeds:state.strategy.stakeholderNeeds,swotStrengths:state.swot.strengths,swotWeaknesses:state.swot.weaknesses,swotOpportunities:state.swot.opportunities,swotThreats:state.swot.threats,swotStrategies:state.swot.strategies,monitoringFrequency:state.monitoring.monitoringFrequency,evaluationFrequency:state.monitoring.evaluationFrequency,evaluationActors:state.monitoring.evaluationActors,followUpMechanism:state.monitoring.followUpMechanism,includeApproval:state.output.includeApproval,includeOutstanding:state.output.includeOutstanding,approvalRole:state.output.approvalRole,approvalName:state.output.approvalName,approvalPlace:state.output.approvalPlace,approvalDate:state.output.approvalDate};
    Object.entries(map).forEach(([id,val])=>{if($(id))$(id).value=val??'';}); $('approvalFields').classList.toggle('hidden',state.output.includeApproval!=='YES'); renderSwotUi();
  }
  ...
  function renderSwotUi() {
    if (!$('swotFields')) return;
    const active=state.swot.mode&&state.swot.mode!=='NONE'; $('swotFields').classList.toggle('hidden',!active);
    $('swotEvidenceWarning').classList.toggle('hidden',!(state.swot.mode==='AI'&&evidenceLevel()==='NONE'));
  }
  ...
  function validate() {
    ...
    if(state.swot.mode==='AI'&&evidenceLevel()==='NONE')add('warning','SWOT AI belum memiliki evidence yang cukup','AI hanya boleh mengelompokkan fakta/context yang tersedia dan tidak akan menciptakan faktor SWOT baru.',3);
    if(state.swot.mode==='USER'&&![state.swot.strengths,state.swot.weaknesses,state.swot.opportunities,state.swot.threats].some(nonEmpty))add('info','SWOT diaktifkan tetapi belum diisi','SWOT bersifat opsional. Isi hanya bila satuan memang memiliki analisis SWOT.',3);
  ...
  function buildSwotBlock() {
    if(!state.swot.mode||state.swot.mode==='NONE')return '';
    const lines=[]; lines.push(`Mode SWOT: ${state.swot.mode==='USER'?'USER — analisis SWOT diberikan pengguna':'AI ASSIST — hanya mengelompokkan/sintesis data yang tersedia'}`);
    push(lines,'Strengths / Kekuatan Internal',state.swot.strengths);push(lines,'Weaknesses / Kelemahan Internal',state.swot.weaknesses);push(lines,'Opportunities / Peluang Eksternal',state.swot.opportunities);push(lines,'Threats / Tantangan-Ancaman Eksternal',state.swot.threats);
    lines.push(`Strategi SO–WO–ST–WT: ${state.swot.strategies==='YES'?'Boleh diusulkan AI — wajib berbasis faktor SWOT yang didukung data dan diberi label USULAN STRATEGI — PERLU VALIDASI':'Tidak diminta'}`); return lines.join('\n');
  }
  ...
  function outputRequirements() {
    ...
    const swotOds=state.swot.mode!=='NONE'?'Pada BAB III sertakan Analisis SWOT sebagai bagian opsional, lengkap dengan dasar data/sumber. SWOT tidak menggantikan evidence dan tidak boleh diperlakukan sebagai sumber fakta baru.':'Jangan membuat bagian SWOT karena pengguna tidak mengaktifkannya.';
    return `${depthText}
${modeText}
${approval}
${outstanding}
...
BAB III EVALUASI DIRI DAN ANALISIS KONDISI [termasuk SWOT bila digunakan]
...
Gunakan tabel utama secara proporsional untuk sumber/temuan evaluasi, prioritas-sasaran, matriks program empat tahun, ringkasan bidang pengelolaan bila perlu, dan monitoring-evaluasi.`;
  ...
  function compilePrompt() {
    ...
    const level=evidenceLevel(),evidence=buildEvidenceBlock(),swot=buildSwotBlock(),existing=buildExistingPlanned(),analysis=buildAnalysisObjectives(),programs=buildProgramsRoadmap(),management=buildManagementMonev(),conflictText=buildConflictRegister();
    const blocks=[];
    blocks.push(`==================================================
ROLE & TASK
==================================================
...
13. SWOT bersifat opsional. Bila digunakan, SWOT hanya boleh bersumber dari Facts, Evidence, dan Context yang tersedia; jangan menciptakan kekuatan, kelemahan, peluang, ancaman, mitra, bantuan, sumber daya, atau risiko eksternal yang tidak diberikan pengguna.`);
    blocks.push(`==================================================
EVIDENCE SUFFICIENCY
==================================================
Evidence Level: ${level}

Jika NONE: jangan membuat klaim baru tentang masalah aktual, kekuatan, akar masalah, atau prioritas aktual satuan di luar fakta/context yang secara eksplisit diberikan pengguna; batasi analisis baru pada struktur, kebutuhan data, dan alternatif umum yang jelas dilabeli sebagai bahan pertimbangan.
...
SWOT adalah alat analisis tambahan, bukan format wajib RKJM dan bukan sumber fakta baru. Jika mode AI digunakan, hanya klasifikasikan/sintesis informasi yang sudah terdapat pada FACTS, EVIDENCE, dan CONTEXT. Jangan menciptakan Strength, Weakness, Opportunity, Threat, mitra, program bantuan, sumber daya, kondisi masyarakat, atau risiko eksternal yang tidak diberikan pengguna. Jika suatu kategori tidak memiliki dasar, kosongkan atau nyatakan data belum tersedia. Strategi SO–WO–ST–WT, bila diminta, merupakan USULAN STRATEGI — PERLU VALIDASI.`);
...
==================================================
Sebelum final, audit REGULATORY, FACTUAL, EXISTING/PLANNED/PROPOSED, NUMERICAL/FUNDING, EVIDENCE, SWOT (jika aktif), COHERENCE, SCOPE, OUTPUT, dan TECHNICAL.

Pastikan kekuatan analisis sesuai Evidence Level; konflik evidence tidak diselesaikan dengan asumsi; planned/proposed tidak menjadi existing; angka yang bukan fakta tidak berubah menjadi fakta resmi; SWOT (jika digunakan) tetap evidence-bound dan tidak menciptakan fakta baru; output tetap RKJM; istilah branch konsisten; dan tidak ada token teknis atau nama variabel internal pada output. Jika ditemukan pelanggaran, perbaiki sebelum final.`);
    blocks.push(`---
Prompt Generator: PG RKJM Edumind | App ${APP_VERSION} | Master Prompt ${MASTER_PROMPT_VERSION} | Regulatory Profile ${REG_PROFILE_VERSION} | Regulatory last verified ${REG_LAST_VERIFIED}`);
  ...
  function recoverPreviousDraft(){
    const archived=loadArchivedDraft();
    if(!archived){toast('Tidak ada cadangan draft sebelumnya.');updateArchiveMenu();return;}
  ...
  function showAbout(){openModal('Tentang PG RKJM',`<p><strong>PG RKJM — Prompt Generator Rencana Kerja Jangka Menengah</strong></p><p class="subtle">Versi aplikasi ${APP_VERSION}<br>Master Prompt ${MASTER_PROMPT_VERSION}<br>Regulatory Profile ${REG_PROFILE_VERSION}<br>Blueprint ${BLUEPRINT_VERSION}</p><p>PG merupakan aplikasi frontend statis. Data draft disimpan otomatis di browser/perangkat melalui localStorage dengan salinan cadangan lokal. Gunakan Ekspor Cadangan Draft bila ingin menyimpan salinan file JSON di luar browser. Hasil akhir tetap perlu ditelaah dan ditetapkan oleh satuan pendidikan.</p>`,`<button class="btn btn-primary" id="aboutClose">Tutup</button>`);$('aboutClose').addEventListener('click',closeModal)}
  ...
  window.__PG_RKJM_QA__ = {
    ...
    swot: buildSwotBlock,
    loadDraft, persistDraft,
    clearDraft, archiveActiveDraft, loadArchivedDraft, enterApp, renderDraftRecovery
  };
