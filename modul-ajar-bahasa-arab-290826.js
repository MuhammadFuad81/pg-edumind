(function () {
  ...
  *{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;background:#0f172a;color:#0f172a;font-family:Inter,ui-sans-serif,system-ui,-apple-system,"Segoe UI",sans-serif;-webkit-user-select:none;user-select:none}
  ...
  .control{width:100%;border:1px solid #cbd5e1;border-radius:9px;padding:10px 11px;background:#f8fafc;color:#0f172a;outline:none;transition:.15s}.control:focus{border-color:#1e40af;box-shadow:0 0 0 3px #dbeafe}.control[readonly]{background:#e2e8f0;color:#475569;cursor:not-allowed}.textarea{resize:vertical;min-height:72px;line-height:1.5}.cp{min-height:150px;background:#fff!important;border:2px solid #f59e0b}.invalid{border-color:#dc2626!important;box-shadow:0 0 0 3px #fee2e2!important}
  ...
  var example = {
    ...
    lintas:'Pendidikan Pancasila dan Informatika', topik_pembelajaran:'Dialog perkenalan (الحوار في التعارف)', metode:'Tanya jawab, pemodelan, latihan pelafalan, dialog berpasangan, penugasan',
    kemitraan:'✓ Lingkungan sekolah: teman sebaya dan guru.\n✓ Lingkungan luar sekolah: orang tua sebagai mitra latihan dialog.', digital:'✓ Audio pelafalan kosakata dan dialog.\n✓ Presentasi visual/kartu kosakata digital.',
  ...
  function currentData() {
    ...
    data.profil = selected('profil'); data.lampiran = selected('lampiran'); return data;
  }
  function save() { try { localStorage.setItem(STORAGE_KEY, JSON.stringify(currentData())); } catch (e) {} }
  function restore() { try { var raw = localStorage.getItem(STORAGE_KEY); if (!raw) return false; var data = JSON.parse(raw); setValues(data, false); ['profil','lampiran'].forEach(function(name){ if(!data[name])return; document.querySelectorAll('input[name="'+name+'"]').forEach(function(cb){cb.checked=data[name].indexOf(cb.value)>=0;}); }); return true; } catch(e){ return false; } }
  ...
  function generatePrompt() {
    ...
    var attachments = selected('lampiran');
    ...
    var lines = [
      ...
      '- Sarana dan Prasarana (pertahankan a, b, c, d):\n' + value('sarpras'),
      '',
      ...
      '5. Jangan mengarang ayat, hadis, sumber, data sekolah, NIP, tautan, atau CP. Bila data tidak tersedia, gunakan penanda [Belum diisi] secara terbatas dan jangan membuat fakta baru.',
