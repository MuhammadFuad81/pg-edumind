/* Edumind Academy | BK V1 Lite R05 | 17 September 2026
...
(function (global) {
  'use strict';
  const APP = 'edumind-bk-lite', VERSION = '1.0.0-beta.5-r05', SCHEMA = 1;
  ...
  const ZONES = {'Asia/Jakarta':'WIB (Waktu Indonesia Barat)','Asia/Makassar':'WITA (Waktu Indonesia Tengah)','Asia/Jayapura':'WIT (Waktu Indonesia Timur)'};
...
/* Edumind BK R05 — gerbang akses dan proteksi sumber RINGAN.
 * Ini BUKAN autentikasi server, otorisasi sekolah, atau enkripsi data.
 * Pemeriksa kredensial ada di browser dan dapat dilewati. Hash hanya
 * menghindari penulisan password secara literal, bukan menyembunyikan rahasia.
 * Tidak mengirim isian/sandi/data sekolah ke jaringan; logo hanya aset statis.
 */
(function(global){
  'use strict';
  const SESSION_KEY='edumind.bk.lite.access.r05';
  const ATTEMPT_KEY=SESSION_KEY+'.attempts';
  const EXPECTED='f2c8363bdec7da9d7cdbd359dab2b76507e17046f9069681f99b57cfd2f3a65d';
  const LOGO='https://growva.biz.id/gambarbebas/20260621-082723_Logo%20Edumind%20Academy%20-%20Terbaru2026%20%5Bputih%5D.png';
  let open=false,started=false,root,host,hooks={},start,working=false,attempts={count:0,until:0};
  let countdown=null,noticeTimer=null,resizeTimer=null,devtoolsShown=false;
  const read=(key)=>{try{return JSON.parse(sessionStorage.getItem(key)||'null');}catch(e){return null;}};
  const write=(key,value)=>{try{value===null?sessionStorage.removeItem(key):sessionStorage.setItem(key,JSON.stringify(value));return true;}catch(e){return false;}};
  const HIDDEN_FACE=`<svg viewBox="0 0 40 40" aria-hidden="true" focusable="false"><circle cx="20" cy="20" r="16" fill="#f2ce9a" stroke="#9c6b37" stroke-width="1.3"/><path d="M7 17C6 3 31 1 34 16L27 10L15 13Z" fill="#334640"/><path d="M16 28q4 3 8 0" fill="none" stroke="#9c6b37" stroke-width="1.5" stroke-linecap="round"/><path d="M4 26l5-11q1-2 3-1l8 6v5l-9 6Z" fill="#e4b77f" stroke="#9c6b37" stroke-width="1.2"/><path d="M36 26l-5-11q-1-2-3-1l-8 6v5l9 6Z" fill="#e4b77f" stroke="#9c6b37" stroke-width="1.2"/><path d="M10 18l8 5M8 22l9 4M30 18l-8 5M32 22l-9 4" stroke="#bb8955" stroke-width="1.2" stroke-linecap="round"/></svg>`;
  const VISIBLE_FACE=`<svg viewBox="0 0 40 40" aria-hidden="true" focusable="false"><circle cx="20" cy="20" r="16" fill="#f2ce9a" stroke="#9c6b37" stroke-width="1.3"/><path d="M7 17C6 3 31 1 34 16L27 10L15 13Z" fill="#334640"/><circle cx="14" cy="20" r="1.8" fill="#263c36"/><circle cx="26" cy="20" r="1.8" fill="#263c36"/><path d="M16 28q4 3 8 0" fill="none" stroke="#9c6b37" stroke-width="1.5" stroke-linecap="round"/></svg>`;
  function logoHTML(){return `<span class="bk-official-logo"><img src="${LOGO}" alt="Edumind Academy" referrerpolicy="no-referrer" draggable="false"><span class="bk-logo-fallback" hidden>Edumind<br><strong>Academy</strong></span></span>`;}
  // SHA-256 fallback: keeps the lightweight gate usable without WebCrypto.
  // Does not make a client-side check into secure authentication.
  function sha256(text){
    const bytes=new TextEncoder().encode(text),len=Math.ceil((bytes.length+9)/64)*64;
    const data=new Uint8Array(len);data.set(bytes);data[bytes.length]=128;
    const dv=new DataView(data.buffer),bits=bytes.length*8;
    dv.setUint32(len-8,Math.floor(bits/4294967296));dv.setUint32(len-4,bits>>>0);
    ...
    for(let off=0;off<len;off+=64){
      ...
      for(let i=0;i<64;i++){const q=(t+(r(e,6)^r(e,11)^r(e,25))+((e&f)^(~e&g))+k[i]+w[i])>>>0;const v=((r(a,2)^r(a,13)^r(a,22))+((a&b)^(a&c)^(b&c)))>>>0;t=g;g=f;f=e;e=(d+q)>>>0;d=c;c=b;b=a;a=(q+v)>>>0;}
      [a,b,c,d,e,f,g,t].forEach((v,i)=>h[i]=(h[i]+v)>>>0);
    }
    return h.map(v=>v.toString(16).padStart(8,'0')).join('');
  }
  function isEditable(target){return !!(target instanceof Element&&target.closest('input,textarea,[contenteditable="true"],[data-allow-copy],.bk-print-preview'));}
  function belongs(target){return !!(target instanceof Node&&(root.contains(target)||host.contains(target)));}
  function softNotice(text){
    let n=document.getElementById('bk-soft-notice');
    if(!n){n=document.createElement('div');n.id='bk-soft-notice';n.setAttribute('role','status');document.body.appendChild(n);}
    n.textContent=text;n.hidden=false;clearTimeout(noticeTimer);noticeTimer=setTimeout(()=>n.hidden=true,5200);
  }
  function installProtection(){
    for(const type of ['contextmenu','copy','cut','selectstart','dragstart'])document.addEventListener(type,ev=>{
      if(!belongs(ev.target)||isEditable(ev.target))return;
      ev.preventDefault();
      if(type==='copy'||type==='contextmenu')softNotice('Gunakan menu unduhan atau cetak yang tersedia. Isian formulir tetap dapat disalin dan ditempel.');
    },true);
    document.addEventListener('keydown',ev=>{
      if(!belongs(ev.target)&&ev.target!==document.body&&ev.target!==document.documentElement)return;
      const key=String(ev.key||'').toLowerCase(),mod=ev.ctrlKey||ev.metaKey;
      const blocked=key==='f12'||(mod&&(key==='u'||key==='s'))||(mod&&ev.shiftKey&&['i','j','c'].includes(key))||(ev.metaKey&&ev.altKey&&['i','j','c','u'].includes(key));
      if(blocked){ev.preventDefault();ev.stopPropagation();softNotice('Gunakan fitur aplikasi yang tersedia. Cetak laporan dan unduhan data tetap dapat digunakan.');}
    },true);
    document.addEventListener('error',ev=>{
      const img=ev.target;if(img instanceof HTMLImageElement&&img.closest('.bk-official-logo')){
        img.hidden=true;const fallback=img.parentElement.querySelector('.bk-logo-fallback');if(fallback)fallback.hidden=false;
      }
    },true);
    // Heuristic only. Toolbars/sidebars can trigger it; never lock, reload,
    // erase data, or claim certain detection. Skip phones and embedded frames.
    function checkViewport(){
      if(devtoolsShown||global.self!==global.top||!global.matchMedia('(pointer:fine)').matches||global.innerWidth<600)return;
      const dw=global.outerWidth-global.innerWidth,dh=global.outerHeight-global.innerHeight;
      if(global.outerWidth>0&&global.outerHeight>0&&(dw>260||dh>300)){
        devtoolsShown=true;const n=document.createElement('div');n.id='bk-inspection-notice';n.setAttribute('role','status');
        const text=document.createElement('span');text.textContent='Pemeriksaan halaman mungkin sedang aktif. Gunakan aplikasi melalui tampilan yang tersedia. Peringatan ini tidak mengubah data.';
        const close=document.createElement('button');close.type='button';close.textContent='Tutup';close.onclick=()=>n.remove();
        n.append(text,close);document.body.appendChild(n);
      }
    }
    global.addEventListener('resize',()=>{clearTimeout(resizeTimer);resizeTimer=setTimeout(checkViewport,1600);});setTimeout(checkViewport,2200);
  }
  function showError(message){const e=host.querySelector('#bk-login-error');e.textContent=message;e.hidden=!message;}
  function updateAttempts(){
    clearInterval(countdown);const button=host.querySelector('button[type="submit"]');
    const tick=()=>{const secs=Math.max(0,Math.ceil((attempts.until-Date.now())/1000));button.disabled=working||secs>0;
      if(secs)showError(`Terlalu banyak percobaan. Tunggu ${secs} detik, lalu coba kembali.`);
      else if(attempts.until){attempts={count:0,until:0};write(ATTEMPT_KEY,attempts);showError('Silakan coba masuk kembali.');clearInterval(countdown);}
    };tick();if(attempts.until>Date.now())countdown=setInterval(tick,1000);
  }
  function renderLogin(message=''){
    host.innerHTML=`<section class="bk-access-shell"><div class="bk-access-card"><div class="bk-access-brand">${logoHTML()}</div><div class="bk-access-kicker">Aplikasi BK</div><h1>Manajemen Program BK<br>Sekolah Islam</h1><p class="bk-access-intro">Masuk untuk melanjutkan pendampingan siswa.</p><form id="bk-login-form"><label for="bk-login-user">Username<input id="bk-login-user" name="username" autocomplete="username" autocapitalize="none" spellcheck="false" placeholder="Masukkan username" required maxlength="80"></label><label for="bk-login-password">Password<div class="bk-password-row"><input id="bk-login-password" name="password" type="password" autocomplete="current-password" placeholder="Masukkan password" required maxlength="128"><button id="bk-password-toggle" type="button" aria-label="Tampilkan password" aria-pressed="false" title="Tampilkan password">${HIDDEN_FACE}<span>Lihat</span></button></div></label><div id="bk-login-error" role="alert" hidden></div><button type="submit" class="bk-access-submit">Masuk Sistem <span aria-hidden="true">→</span></button></form><p class="bk-access-personal">Akses ini bersifat <strong>PRIBADI</strong> untuk pengguna resmi Edumind Academy. Mari sama-sama menjaga keberkahan ilmu.</p><p class="bk-access-hint">Gunakan perangkat pribadi. Setelah selesai, pilih <strong>Keluar</strong>.</p><div class="bk-access-footer">Edumind Academy · V1 Lite R05</div></div></section>`;
    showError(message);updateAttempts();
    const field=host.querySelector('#bk-login-password'),toggle=host.querySelector('#bk-password-toggle');
    toggle.addEventListener('click',()=>{const visible=field.type==='password';field.type=visible?'text':'password';toggle.innerHTML=(visible?VISIBLE_FACE:HIDDEN_FACE)+`<span>${visible?'Tutup':'Lihat'}</span>`;toggle.setAttribute('aria-pressed',String(visible));toggle.setAttribute('aria-label',visible?'Sembunyikan password':'Tampilkan password');toggle.title=toggle.getAttribute('aria-label');field.focus({preventScroll:true});});
    host.querySelector('#bk-login-form').addEventListener('submit',async ev=>{
      ev.preventDefault();if(working||attempts.until>Date.now())return;
      const form=ev.currentTarget;if(!form.reportValidity())return;
      working=true;host.querySelector('button[type="submit"]').disabled=true;
      try{
        const user=host.querySelector('#bk-login-user').value.trim();
        const ok=sha256(user+'\u0000'+field.value)===EXPECTED;
        field.value='';field.type='password';toggle.innerHTML=HIDDEN_FACE+'<span>Lihat</span>';toggle.setAttribute('aria-pressed','false');toggle.setAttribute('aria-label','Tampilkan password');toggle.title='Tampilkan password';
        if(!ok){attempts.count++;if(attempts.count>=5)attempts.until=Date.now()+60000;write(ATTEMPT_KEY,attempts);showError('Username atau password belum sesuai. Periksa kembali huruf dan tandanya.');field.focus();return;}
        attempts={count:0,until:0};write(ATTEMPT_KEY,null);write(SESSION_KEY,{version:1,issuedAt:Date.now()});await unlock();
      }catch(e){showError('Aplikasi belum dapat dibuka. Coba muat ulang halaman; jangan menghapus data yang sudah tersimpan.');}
      finally{working=false;if(!open)updateAttempts();}
    });
  }
  async function unlock(){
    open=true;root.dataset.access='open';host.hidden=true;root.hidden=false;root.inert=false;
    if(!started){started=true;await start();}else if(hooks.onResume)await hooks.onResume();
    const pass=host.querySelector('#bk-login-password');if(pass)pass.value='';
    root.querySelector('h1')?.setAttribute('tabindex','-1');root.querySelector('h1')?.focus({preventScroll:true});
  }
  function logout(){
    if(!open)return false;if(hooks.beforeLogout&&hooks.beforeLogout()===false)return false;
    open=false;write(SESSION_KEY,null);root.dataset.access='closed';root.hidden=true;root.inert=true;host.hidden=false;
    const toast=root.querySelector('.bk-toast');if(toast)toast.hidden=true;
    renderLogin('Anda sudah keluar. Data yang telah disimpan tetap ada di perangkat ini.');global.scrollTo(0,0);host.querySelector('#bk-login-user')?.focus({preventScroll:true});return true;
  }
  function launch(target,callback){
    root=target;start=callback;root.hidden=true;root.inert=true;root.dataset.access='closed';
    host=document.createElement('div');host.id='edumind-bk-access';root.before(host);
    const a=read(ATTEMPT_KEY);if(a&&Number.isInteger(a.count)&&Number.isFinite(a.until))attempts={count:Math.max(0,a.count),until:a.until};
    renderLogin();installProtection();root.dataset.ready='1';
    const s=read(SESSION_KEY);if(s&&s.version===1&&Number.isFinite(s.issuedAt))unlock();
  }
  global.BKAccess={launch,logout,logoHTML,isOpen:()=>open,setHooks:value=>{hooks=value;}};
...
(function(){
  ...
  const Gate=window.BKAccess;
  Gate.launch(root,async function startBKApp(){
  ...
  const CAUTION='Gunakan perangkat pribadi dan cadangkan data secara berkala. Halaman masuk hanya pembatas akses sederhana; bukan akun pribadi sekolah dan tidak mengunci berkas data. Orang yang mengakses perangkat atau berkas cadangan Anda dapat membaca isinya. Jangan masukkan informasi pribadi yang sangat sensitif.';
  ...
  root.addEventListener('click',async ev=>{
    ...
    try{
      if(a==='logout'){Gate.logout();return;}
  ...
  async function openWorkspace(){
    fatal='';pendingCSV=null;pendingRestore=null;lastReport='';page='home';filter={search:'',classId:''};
    readInitial();if(!fatal&&migrationPending){try{await commit(D);migrationPending=false;}catch(e){fatal='Pembaruan semester belum dapat disimpan. Data lama tidak diganti. '+userMessage(e);}}
    render();root.dataset.appReady='1';
  }
  Gate.setHooks({beforeLogout:()=>{if(!closeDialog())return false;clearTimeout(toastTimer);pendingCSV=null;pendingRestore=null;lastReport='';return true;},onResume:openWorkspace});
  await openWorkspace();
  });
})();
